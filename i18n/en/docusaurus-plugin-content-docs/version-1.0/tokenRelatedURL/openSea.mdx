...
For both `tradeType=fixedPrice` and `tradeType=auction`, the NFT is not published until it is bought or successfully withdrawn.
To check whether the Item has been bought or withdrawn, please refer to `Item.buyerAddress` for a value.
